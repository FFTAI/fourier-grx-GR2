...
由于这个需求比较特殊，需要既用到机器人的高层控制接口 （[User 接口](/fourier-grx-GR2/docs/reference/user)），又要用到机器人的关节层面底层控制接口（[Developer 接口](/fourier-grx-GR2/docs/reference/developer)）。
...
1. 创建机器人的启动配置文件：
    - 打开文件夹 `~/fourier-grx/config/n1` 文件夹，找到 `config_GR2__release.yaml` 文件。
    - 将该文件复制一份，命名为 `config_GR2__mobile.yaml`。

2. 修改配置文件：
    - 打开 `config_GR2__mobile.yaml` 文件。
...
5. 手柄控制：
    - 手柄遥控程序可以参考 fourier-grx SDK **User** 接口中的 [demo_walk.py](https://github.com/FFTAI/Wiki-GRx-Deploy/blob/FourierGR2/user/demo_walk.py) 程序。
...
1. 创建机器人的启动配置文件：
    - 打开文件夹 `~/fourier-grx/config/n1` 文件夹，找到 `config_GR2__release.yaml` 文件。
    - 将该文件复制一份，命名为 `config_GR2__upper.yaml`。

2. 修改配置文件：
    - 打开 `config_GR2__upper.yaml` 文件。
...
3. 启动上肢控制程序：
    - 上肢关节控制程序可以参考 fourier-grx SDK **Developer** 接口中的 [demo_ready_state.py](https://github.com/FFTAI/Wiki-GRx-Deploy/blob/FourierGR2/developer/demo_ready_state.py) 程序。
    - 启动时需指定配置文件为上肢控制专用配置文件，可以让机器人关节上肢部分移动到准备状态位置。

    ```bash
    # 在 终端3 中执行以下命令
    python demo_ready_state.py --config=config_GR2__upper.yaml
    ```
