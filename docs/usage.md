...
本文档介绍了使用 Fourier-GRX-GR2 SDK 时的常用操作流程和注意事项。

## 固件安装和更新

Fourier-GRX-GR2 SDK 安装文件请从 [固件更新](/fourier-grx-GR2/docs/update) 下载。
下载完后，运行下面的指令完成安装过程：
...
| 机器人型号 | 机器人版本 | 测试机型 | [运行模式](/fourier-grx-GR2/docs/usage#运行模式选择) | 适配的机器人型号 |
|-------|-------|------|--------------------------------------------|----------|
| GR2   | T3    | 否    | 开发者模式                                      | GR2      |
...
> ℹ️ **说明**：
>
> 关于任务和模块的具体描述，以及更多任务功能信息，请参阅 [参考指南 User 接口](/fourier-grx-GR2/docs/reference/user) 的 `task/client 接口协议 (指令信息)` 章节的内容。

目前 Fourier GR2 在调试模式下，可通过手柄调用的内部主要功能包括：
...
- 开发接口：[参考指南](/fourier-grx-GR2/docs/reference)
...
5. 数据分析：
    ...
    - 数据文件中包含时间戳、关节角度、速度等信息，方便进行后续分析。
        ...
        - 关节序列参考 [关节序列](/fourier-grx-GR2/docs/reference/joint_sequence) 文档。
